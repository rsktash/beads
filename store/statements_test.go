...
func TestMigrationVersion4Applied(t *testing.T) {
	...
	if len(status) != 11 {
		t.Fatalf("expected 11 migrations, got %d: %+v", len(status), status)
	}
...
func TestMigrationIdempotent(t *testing.T) {
	...
	if len(status) != 11 {
		t.Fatalf("expected 11 after second open, got %d", len(status))
	}
}
...
func TestPostgresStatements(t *testing.T) {
	...
	if len(status) != 11 {
		t.Fatalf("expected 11 migrations pg, got %d", len(status))
	}
	var cnt int
